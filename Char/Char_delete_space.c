/* Программа удаления "лишних" пробелов.

int a, b; // Объявление переменных a и b, которые будут использоваться для хранения 
символов
    
b = 's'; // Инициализация переменной b символом 's'

Начало цикла, который будет выполняться, пока вводимые данные не равны 
константе EOF (конец файла)
